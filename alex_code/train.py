...
def train_model(config, checkpoint_dir = None):
	...
	restore_parameters(cmlp, best_model)
			...
			# if len(loss_list) > 1:
			# 	variable_usage_crit = abs(variable_usage_list[-1]-variable_usage_list[-2])/variable_usage_list[-2]
			# 	loss_crit = (loss_list[-2]-loss_list[-1])/loss_list[-2]

			# 	if verbose:
					
			# 		print('Lam={}: Iter {}, {} sec'.format(lam,it+1,np.round(time.time()-start,2)),
			# 			  '-----','Loss: %.2f' % loss,', Variable usage = %.2f%%' % (100 * variable_usage),
			# 			  '|||','%.3f' % loss_crit,'%.3f' % variable_usage_crit)

			# 	if variable_usage_crit < tol: # or loss_crit < tol:
			# 		print('EARLY STOP: Lam={}, Iter {}'.format(lam,it + 1),
			# 			  '%.3f' % loss_crit,'%.3f' % variable_usage_crit)
			# 		break
